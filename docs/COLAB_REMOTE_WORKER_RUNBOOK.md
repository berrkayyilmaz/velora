...
6. Backend downloads the generated artifact from the worker over HTTP and stores it locally.
7. Smoke tooling verifies the final job, result metadata, and downloaded artifact metadata.
...
export VELORA_AI_WORKER_OUTPUT_DIRECTORY="/content/velora/ai-worker/data/output"
...
$env:TRY_ON_REMOTE_WORKER_ARTIFACT_PATH="/jobs/{workerJobId}/artifact"
...
The worker artifact path must point at the Colab worker artifact endpoint. The backend downloads generated bytes from `TRY_ON_REMOTE_WORKER_ARTIFACT_PATH` and writes them to the local `TRY_ON_SMOKE_OUTPUT_DIRECTORY`.
...
- Confirm `TRY_ON_REMOTE_WORKER_ARTIFACT_PATH="/jobs/{workerJobId}/artifact"`.
- Confirm the worker's `VELORA_AI_WORKER_OUTPUT_DIRECTORY` contains the generated file.
- Confirm the worker result metadata path resolves under `VELORA_AI_WORKER_OUTPUT_DIRECTORY`.
- Confirm the local backend can write to `TRY_ON_SMOKE_OUTPUT_DIRECTORY`.
- Confirm the tunnel is still active and publicly serves `/jobs/<WORKER_JOB_ID>/artifact`.

## Known Research Limitation

Remote artifact transfer now works through the worker artifact endpoint for research smoke testing. This is still temporary infrastructure: generated files are stored locally in the Colab runtime, exposed through a temporary tunnel, and should be removed after each experiment.

For production, replace temporary tunnel storage with authenticated object storage or a hardened worker artifact service.
